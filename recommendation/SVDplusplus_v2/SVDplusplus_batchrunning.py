...
tf.app.flags.DEFINE_integer('Epochs',15,'number of maximum epochs to train the model')
...
class SVDplusplus():
    def __init__(self, allfile, trainfile, testfile, max_epoch=15, latentFactorNum=20,alpha_u=0.01,alpha_v=0.01,alpha_w=0.01,beta_u=0.01,beta_v=0.01,learning_rate=0.01):
    ...
    def load_existing_matrix_and_recommend(self, user=1, topK=10):
        time1=time.time()
        pred_matrix_path_new = pred_matrix_path[:-4]+'_'+str(self.latentFactorNum)+'.npy'
        loaded_weight_matrix=np.load(pred_matrix_path_new)
        reco_list = []
    ...
    def evaluation(self,topK=10):
        ...
        '''
        df_hit, df_hit_count, n_users = self.merge_ranking_true_pred(
        ...
        '''
        df_hit=pd.DataFrame(self.get_top_k_items(
        dataframe=self.test_df,col_user='user_id',col_rating='rating',k=topK),
        columns=['user_id','item_id','rating'])

        df_hit['rank']=df_hit.groupby('user_id')['rating'].rank(method="first", ascending=False)
        def TPFN(self,df_hit,topK=10,user_id=1):
            df_hit_truth = df_hit
            df_hit_truth_for_given_user = df_hit.loc[df_hit['user_id']==user_id]['item_id']
            truth_topK = list(set(df_hit_truth_for_given_user.values))
            predict_topK = [obj[0] for obj in self.load_existing_matrix_and_recommend(user=user_id, topK=topK)]

            #print(predict_topK,truth_topK)
            TP_count=0
            FN_count=0
            P_count=len(predict_topK)
            for truth_item in truth_topK:
                if truth_item in predict_topK:
                    TP_count+=1
                else:
                    FN_count+=1
            return TP_count, FN_count, P_count
        def MAE():
            return self.MAE
        def RMSE():
            return self.RMSE
        def precision_at_top_K(topK):
            # measures initializer
            TP_count, FN_count, P_count= [0 for i in range(3)]
            # Iterate simultaneously in the pred/truth dataset
            for user_id in list(set(df_hit['user_id'].values)):
                TP_count+=TPFN(self,df_hit=df_hit,topK=10,user_id=user_id)[0]
                FN_count+=TPFN(self,df_hit=df_hit,topK=10,user_id=user_id)[1]
                P_count+=TPFN(self,df_hit=df_hit,topK=10,user_id=user_id)[2]
            if df_hit.shape[0] == 0:
                return 0.0
            return TP_count/P_count
            #return (df_hit_count["hit"] / topK).sum() / n_users
        def recall_at_topK(topK):
            TP_count, FN_count = [0 for i in range(2)]
            for user_id in list(set(df_hit['user_id'].values)):
                TP_count+=TPFN(self,df_hit=df_hit,topK=10,user_id=user_id)[0]
                FN_count+=TPFN(self,df_hit=df_hit,topK=10,user_id=user_id)[1]
            if df_hit.shape[0] == 0:
        ...
        self.precision = precision_at_top_K(topK)
        self.recall = recall_at_topK(topK)
        return MAE,RMSE,precision_at_top_K(topK),recall_at_topK(topK)
    ...
    def report_generation(self):
        ...
        print('Evaluation Report:',content)
        f.write(content)
        f.close()

def single_task(latent):
    print('Running task {0} (pid {1}), SVD++ with {2} latentFactors'.format(latent//2,os.getpid(),latent))
    start = time.time()
    s = SVDplusplus(allfile=udata_path, trainfile=u1base_path, testfile=u1test_path, latentFactorNum=latent)
    ...
    '''
    s.train(iterTimes=s.max_epoch)
    s.evaluation()
    s.report_generation()
    end = time.time()
    print('Task %d runs %0.2f seconds.' % (latent, (end - start)))
    del s
    gc.collect()

def main(_):
    print('SVD++ batch processing program started. Parent process %s.' % os.getpid())
    p = Pool(4)
    for Num in range(1,50):
        latent = 2 * Num
        p.apply_async(single_task, args=(latent,))
